...
// Fetch stats
$total_employees_stmt = $pdo->query("SELECT COUNT(*) FROM employees");
$total_employees = $total_employees_stmt->fetchColumn();

$total_leave_stmt = $pdo->query("SELECT COUNT(*) FROM leave_requests");
$total_leave_requests = $total_leave_stmt->fetchColumn();
...
<head>
<meta charset="UTF-8">
<title>Admin Dashboard - StaffTrack</title>
...
<p class="text-3xl font-bold text-red-600"><?= $total_employees ?></p>
</div>

<div class="bg-white p-6 rounded-xl shadow-md text-center">
<h3 class="text-lg font-semibold text-gray-700 mb-2">Total Departments</h3>
<p class="text-3xl font-bold text-red-600"><?= $total_departments ?></p>
</div>

<div class="bg-white p-6 rounded-xl shadow-md text-center">
<h3 class="text-lg font-semibold text-gray-700 mb-2">Total Leave Requests</h3>
<p class="text-3xl font-bold text-red-600"><?= $total_leave_requests ?></p>
</div>
</div>
